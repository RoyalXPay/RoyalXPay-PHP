            ...
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Create Sale Quotation</li>
                    </ol>
                    <div class="page-title-right">
                        <a href="<?= site_url('sale-quotations'); ?>" class="btn btn-secondary waves-effect waves-light">
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <div class="container">
                            <form class="custom-validation" method='post' action="<?= site_url('sale-quotations/save') ?>" enctype='multipart/form-data'>
                                <?= \Config\Services::validation()->listErrors(); ?>
                                <?= csrf_field() ?>
                                <?= view('flash_messages'); ?>
                                <h4 class="card-title"><i class="fas fa-code-branch"></i> <?= isset($quotationDetails) ? 'Edit' : 'Create' ?> Sale Quotation</h4>
                                <p class="card-subtitle mb-2 text-muted">Fill in the details below to <?= isset($quotationDetails) ? 'edit' : 'create' ?> a Sale Quotation.</p>

                                <!-- Hidden quotation ID and quotation code (used for editing quotation) -->
                                <input type="hidden" id="quotationId" name="quotation_id" value="<?= $quotationDetails['quotation_id'] ?? ''; ?>">
                                <input type="hidden" id="quotationCode" name="quotation_code" value="<?= $quotationDetails['quotation_code'] ?? ''; ?>">
                                        ...
                                        <select class="form-select" id="userId" name="user_id" required>
                                            ...
                                            <?php foreach ($customers as $customer): ?>
                                                <option value="<?= htmlspecialchars($customer->user_id); ?>"
                                                    <?= ($customer->user_id == ($quotationDetails["user_id"] ?? "")) ? 'selected' : ''; ?>>
                                    ...
                                    <table id="productTable" class="table table-bordered table-striped dt-responsive w-100">
                                        ...
                                        <tbody>
                                            <?php foreach ($products as $product):
                                                ...
                                                $isChecked = isset($quotationDetails['quotationProducts'][$productId]);

                                                // Get the saved quantity and discount, fallback to empty if not in contract
                                                $quantity = $isChecked ? $quotationDetails['quotationProducts'][$productId]['quantity'] : ($isPurchase ? 1 : '');
                                                $discount = $isChecked ? $quotationDetails['quotationProducts'][$productId]['discount'] : '';
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="totalAmount" name="total_amount" value="<?= $quotationDetails['total_price'] ?? ''; ?>" readonly placeholder="Total amount will appear here">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="vat" name="vat" value="<?= $quotationDetails['vat'] ?? ''; ?>" readonly placeholder="VAT will appear here">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="finalPayableAmount" name="final_payable_price" value="<?= $quotationDetails['final_payable_price'] ?? ''; ?>" readonly placeholder="Final Amount will appear here">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="deliveryTimes" name="delivery_time" class="form-select" placeholder="Select delivery time for sale">
                                        ...
                                        <?php $deliveryTime = $quotationDetails["delivery_time"] ?? ""; ?>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="termsConditions" name="terms_conditions" class="form-select" placeholder="Select terms and conditions">
                                        ...
                                        <?php $termsConditions = $quotationDetails['terms_conditions'] ?? ""; ?>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="Enter any additional notes here"><?= $quotationDetails["notes"] ?? ''; ?></textarea>
                                </div>

                                <!-- Modal Footer: Buttons to cancel or save the contract -->
                                <div class="text-end">
                                    <a href="<?= site_url('sale-quotations'); ?>" class="btn btn-outline-secondary">Cancel</a>
