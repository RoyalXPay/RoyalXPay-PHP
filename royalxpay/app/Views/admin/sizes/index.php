                ...
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="page-title-right">
                        <?php if (!empty($permissions) && in_array('add', $permissions)) { ?>
                            <button type="button" data-bs-toggle="modal" data-bs-target="#newSizeModal" class="btn btn-primary waves-effect waves-light">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-4" style="margin-top: 27px;">
                                                ...
                                                <a href="<?= site_url('sizes'); ?>" class="btn btn-secondary waves-effect waves-light mr-1" data-bs-toggle="tooltip" data-bs-placement="top" title="Clear Searched Filters">
                                ...
                                <table id="datatable" class="table table-bordered table-striped dt-responsive w-100">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-sortable="true" class="text-left">Size (Inches)</th>
                                    ...
                                    <tbody>
                                        <?php foreach ($results as $size) { ?>
                                            <tr>
                                                <td><?= ++$startLimit; ?></td>
                                                <td><?= esc($size->size_in_inches); ?> inches</td>
                                                <td>
                                                    <?php
                                                    if (!empty($size->updated_at)) {
                                                        echo date('j M Y, h:i A', strtotime($size->updated_at));
                                                    } else {
                                                        echo date('j M Y, h:i A', strtotime($size->created_at));
                                                    }
                                                    ?>
                                                </td>
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <?php if (!empty($permissions) && in_array('edit', $permissions)) { ?>
                                                            <button type="button" class="btn btn-success m-1 editBtn" data-id="<?= $size->size_id; ?>" data-bs-toggle="tooltip" title="Edit">
                                                                <i class="fas fa-edit"></i>
                                                            </button>
                                                        <?php } ?>
                                                        <?php if (!empty($permissions) && in_array('delete', $permissions)) { ?>
                                                            <button type="button" class="btn btn-danger m-1 dltBtn" data-id="<?= $size->size_id; ?>" data-bs-toggle="tooltip" title="Delete">
...
<!-- Add/Edit Modal -->
<?= $this->include('admin/sizes/create') ?>
...
<script>
    $(document).ready(function() {
        ...
        $('#newSizeModal').on('hidden.bs.modal', function() {
            $('#sizeForm')[0].reset();
            $('#modalMessages').html('');
            $('#sizeId').val('');
            $('#submitButton').text('Save');
            $('#newSizeModalLabel').text('Add Size');
        });

        // Handle form submission for Size
        $('#sizeForm').on('submit', function(e) {
            e.preventDefault();
            var url = "<?= site_url('sizes/save') ?>";
        ...
        $('.editBtn').on('click', function(e) {
            ...
            $.ajax({
                url: '<?= site_url('sizes/edit') ?>/' + sizeId,
                method: 'GET',
                dataType: 'json',
                success: function(data) {
                    $('#sizeId').val(data.size_id);
                    $('#size_in_inches').val(data.size_in_inches);

                    $('#submitButton').text('Update');
                    $('#newSizeModalLabel').text('Edit Size');
        ...
        $('.dltBtn').click(function(e) {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        type: "GET",
                        url: "<?= site_url('sizes/delete/') ?>" + sizeId,
                        success: function(response) {
