                ...
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="page-title-right">
                        <?php if (!empty($permissions) && in_array('add', $permissions)) { ?>
                            <button type="button" data-bs-toggle="modal" data-bs-target="#newProductTypeModal" class="btn btn-primary waves-effect waves-light">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-4" style="margin-top: 27px;">
                                                ...
                                                <a href="<?= site_url('producttypes'); ?>" class="btn btn-secondary waves-effect waves-light mr-1" data-bs-toggle="tooltip" data-bs-placement="top" title="Clear Searched Filters">
                                ...
                                <table id="datatable" class="table table-bordered table-striped dt-responsive w-100">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-sortable="true" class="text-left">Product Type</th>
                                    ...
                                    <tbody>
                                        <?php foreach ($results as $productType) { ?>
                                            <tr>
                                                <td><?= ++$startLimit; ?></td>
                                                <td><?= esc($productType->name); ?></td>
                                                <!-- <td><?= esc($productType->description); ?></td> -->
                                                <td>
                                                    <?php
                                                    if (!empty($productType->updated_at)) {
                                                        echo date('j M Y, h:i A', strtotime($productType->updated_at));
                                                    } else {
                                                        echo date('j M Y, h:i A', strtotime($productType->created_at));
                                                    }
                                                    ?>
                                                </td>
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <?php if (!empty($permissions) && in_array('edit', $permissions)) { ?>
                                                            <button type="button" class="btn btn-success m-1 editBtn" data-id="<?= $productType->id; ?>" data-bs-toggle="tooltip" title="Edit">
                                                                <i class="fas fa-edit"></i>
                                                            </button>
                                                        <?php } ?>
                                                        <?php if (!empty($permissions) && in_array('delete', $permissions)) { ?>
                                                            <button type="button" class="btn btn-danger m-1 dltBtn" data-id="<?= $productType->id; ?>" data-bs-toggle="tooltip" title="Delete">
...
<!-- Add/Edit Modal -->
<?= $this->include('admin/basics/products/create') ?>
...
<script>
    $(document).ready(function() {
        ...
        $('#newProductTypeModal').on('hidden.bs.modal', function() {
            $('#productTypeForm')[0].reset();
            $('#modalMessages').html('');
            $('#productTypeId').val('');
            $('#submitButton').text('Save');
            $('#newProductTypeModalLabel').text('Add Product Type');
        });

        // Handle form submission for Product Type
        $('#productTypeForm').on('submit', function(e) {
            e.preventDefault();
            var url = "<?= site_url('product-types/save') ?>";
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'error') {
                        ...
                        setTimeout(function() {
                            $('#newProductTypeModal').modal('hide');
                            location.reload();
                        }, 2000);
                    }
                },
        ...
        $('.editBtn').on('click', function(e) {
            ...
            $.ajax({
                url: '<?= site_url('product-types/edit') ?>/' + productTypeId,
                method: 'GET',
                dataType: 'json',
                success: function(data) {
                    ...
                    $('#newProductTypeModalLabel').text('Edit Product Type');
        ...
        $('.dltBtn').click(function(e) {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        type: "GET",
                        url: "<?= site_url('product-types/delete/') ?>" + dataID,
                        success: function(response) {
