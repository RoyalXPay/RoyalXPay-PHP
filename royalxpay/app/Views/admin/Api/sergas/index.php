...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Sergas Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/sergas/reports') ?>" class="btn btn-info">Sergas Payment Reports</a>
...
<script>
    ...
    function validateInput(value, type) {
        ...
        switch (type) {
           case 'AccountID': 
            if (!/^[A-Z]{1}\d{5}$/.test(value)) {
                errorEl.text('Account ID should start with a capital letter followed by 5 digits.').show();
                return false;
            }
            break;
            case 'MobileNo':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/sergas/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/sergas/fetch-bill') ?>", {
                    method: 'POST',
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/sergas/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "sergas/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
