...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Upay Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/upay/reports') ?>" class="btn btn-info">upay Payment Reports</a>
        ...
        <div class="card">
            <div class="card-body">
                <form id="upay-payment-form">
                    <div class="row mb-3">
                        <label>Select Search Type</label>
                        <div class="col-md-12 d-flex flex-wrap gap-2">
                            <button type="button" class="btn btn-outline-primary search-type-btn" data-type="AccountID">productFlag</button>
...
<script>
    ...
    function validateInput(value, type) {
        ...
        switch (type) {
           case 'AccountID': 
            if (!value.trim()) {
                errorEl.text('This field cannot be empty.').show();
                return false;
            }
            break;
            case 'MobileNo':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/upay/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/upay/fetch-bill') ?>", {
                    method: 'POST',
                ...
                if (data.resField1) {
                    html += `<tr><th>Customer mobile number</th><td>${data.resField1}</td></tr>`;
                }
                if (data.resField2) {
                    html += `<tr><th>Customer previous balance</th><td>${data.resField2}</td></tr>`;
                }
                if (data.resField3) {
                    html += `<tr><th>Contract number</th><td>${data.resField3}</td></tr>`;
                }
                if (data.resField4) {
                    html += `<tr><th>Building number</th><td>${data.resField4}</td></tr>`;
                }
                if (data.resField5) {
                    html += `<tr><th>Apartment number</th><td>${data.resField5}</td></tr>`;
                }
                // Calculated fields
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/upay/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "upay/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
