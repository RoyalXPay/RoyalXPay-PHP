...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Mawaqif Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/mawaqif/reports') ?>" class="btn btn-info">Mawaqif Payment Reports</a>
        ...
        <div class="card">
            <div class="card-body">
                <form id="mawaqif-payment-form">
                    <div class="row mb-3">
                        <label>Select Search Type</label>
                        <div class="col-md-12 d-flex flex-wrap gap-2">
                            <button type="button" class="btn btn-outline-primary search-type-btn" data-type="MobileNo">Account Number</button>
...
<script>
        ...
        switch (type) {
            case 'AccountID': 
                if ( !/^[0-9]{10}$/.test(value)) {
                    errorEl.text('Account Number should be 9 - 10 digits long.').show();
                    return false;
                }
                break;
            case 'MobileNo':
                if (!/^[0-9]{10}$/.test(value)) {
                    errorEl.text('Phone Number should be 9 - 10 digits long.').show();
                    return false;
                }
                break;
            case 'PersonID':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/mawaqif/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/mawaqif/fetch-bill') ?>", {
                    method: 'POST',
                ...
                const  resField10= data.resField10;
                ...
                if (data.amount) {
                    ...
                    html += `<tr><th>Balance</th><td>${data.amount}</td></tr>`;
                }
                if (data.resField1) {
                    html += `<tr><th>Plate Number</th><td>${data.resField1}</td></tr>`;
                }
                if (data.resField2) {
                    html += `<tr><th>Emirate/Country</th><td>${data.resField2}</td></tr>`;
                }
                if (data.resField3) {
                    html += `<tr><th>Plate Category</th><td>${data.resField3}</td></tr>`;
                }
                if (data.resField4) {
                    html += `<tr><th>Plate Type</th><td>${data.resField4}</td></tr>`;
                }
                if (data.resField5) {
                    html += `<tr><th>Fine Issue Date</th><td>${data.resField5}</td></tr>`;
                }
                if (data.resField6) {
                    html += `<tr><th>Attibute1</th><td>${data.resField6}</td></tr>`;
                }
                if (data.resField7) {
                    html += `<tr><th>Status</th><td>${data.resField7}</td></tr>`;
                }
                if (data.resField8) {
                    html += `<tr><th>Location code</th><td>${data.resField8}</td></tr>`;
                }
                if (data.resField9) {
                    html += `<tr><th>Stage</th><td>${data.resField9}</td></tr>`;
                }
                if (data.resField10) {
                    html += `<tr><th>Invoice number</th><td>${data.resField10}</td></tr>`;
                }
                if (data.resField11) {
                    html += `<tr><th>Discount</th><td>${data.resField11}</td></tr>`;
                }
                if (data.resField12) {
                    html += `<tr><th>Fine Description</th><td>${data.resField12}</td></tr>`;
                }
                
                if (data.providerTransactionId) {
                    html += `<tr><th>Date</th><td>${data.providerTransactionId}</td></tr>`;
                }
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/mawaqif/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    const responseMessage = payResult?.data?.responseMessage || payResult?.message;
        ...
        //             setTimeout(() => {
        //                 window.location.href = "mawaqif/reports";
        //             }, 2000);
        //         } else {
