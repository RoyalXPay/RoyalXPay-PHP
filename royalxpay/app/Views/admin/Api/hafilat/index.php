...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Hafilat Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/hafilat/reports') ?>" class="btn btn-info">Hafilat Payment Reports</a>
...
<script>
        ...
        switch (type) {
            case 'AccountID':
                if ( !/^[0-9]{15}$/.test(value)) {
                    errorEl.text('Account number should be 15 digits.').show();
                    return false;
                }
                break;
            case 'MobileNo':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/hafilat/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/hafilat/fetch-bill') ?>", {
                    method: 'POST',
...
// Show Additional Fields (custName, resField3, resField4, etc.)
if (data.custName) {
    html += `<tr><th>Customer Name</th><td>${data.custName}</td></tr>`;
}
if (data.resField3) {
    html += `<tr><th>Title</th><td>${data.resField3}</td></tr>`;
}
if (data.resField4) {
    html += `<tr><th>Card Status</th><td>${data.resField4}</td></tr>`;
}

// Handle arrayResponse
if (data && Array.isArray(data.arrayResponse) && data.arrayResponse.length > 0) {
    const detail = data.arrayResponse[0];

    const balanceAmount = parseFloat(detail.BalanceAmount || 0);
    const commission = 2;
    const finalAmount = (balanceAmount + commission).toFixed(2);
    const productCode = detail.ProductCode || '';
const itemCode = detail.ItemInfo?.[0]?.ItemCode || '';

    // Show editable Balance if needed
    html += `<tr>
        <th>Balance Amount</th>
        <td><input type="number" min="0" step="0.01" id="editableBalance" class="form-control" value="${balanceAmount.toFixed(2)}" /></td>
    </tr>`;

    // Show Amount in Process if present
    if (detail.AmountInProcess) {
        html += `<tr><th>Amount In Process</th><td>${detail.AmountInProcess}</td></tr>`;
    }

    // Show Product Info
    if (detail.ProductTitle) {
        html += `<tr><th>Product</th><td>${detail.ProductTitle}</td></tr>`;
    }
    if (detail.ProductCode) {
        html += `<tr><th>Product Code</th><td>${detail.ProductCode}</td></tr>`;
    }
    if (detail.ProductCategory) {
        html += `<tr><th>Product Category</th><td>${detail.ProductCategory}</td></tr>`;
    }
    if (detail.ValidityEndDate) {
        html += `<tr><th>Validity End Date</th><td>${detail.ValidityEndDate}</td></tr>`;
    }
    if (detail.ValidityStartDate) {
        html += `<tr><th>Validity Start Date</th><td>${detail.ValidityStartDate}</td></tr>`;
    }
   
    if (detail.TransactionType) {
        html += `<tr><th>Transaction Type</th><td>${detail.TransactionType}</td></tr>`;
    }

    // Add Commission and Final Amount
    ...
    $('#continueBtn').data('payload', {
        ...
        transaction_id: transactionId,
        amount: finalAmount,
        productCode: productCode,  // ✅ use lowercase variable
    itemCode: itemCode,
        balance: balanceAmount.toFixed(2)  // for API paidAmount
    });
}
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/hafilat/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
         ...
         // Normalize response (check if data exists, else fallback to root level)
    const status = payResult?.data?.status || payResult?.status;
    const responseCode = payResult?.data?.responseCode || payResult?.responseCode;
    const responseMessage = payResult?.data?.responseMessage || payResult?.message;

    if (status === 'SUCCESS') {
        Swal.fire({
            icon: 'success',
            title: 'Payment Successful',
            text: responseMessage || 'Payment completed successfully',
        });
        setTimeout(() => {
            window.location.href = "aadc/reports";
        }, 2000);
    } else if (responseCode === '111' || status === 'PENDING') {
        Swal.fire({
            icon: 'warning',
            title: 'Transaction Pending',
            text: responseMessage || 'Your transaction is currently pending. Please check back later.',
        });
        ...
        Swal.fire({
            ...
            text: 'An error occurred. Please contact MBME support.',
        });
    } else if (responseCode === '999') {
        Swal.fire({
            icon: 'error',
            title: 'Server Unreachable',
            text: 'Remote server not reachable. Try again later.',
        });
    } else {
        Swal.fire({
            icon: 'error',
            title: 'Payment Failed',
            text: responseMessage || 'Transaction failed. Please try again.',
        });
    }
        ...
        //             setTimeout(() => {
        //                 window.location.href = "hafilat/reports";
        //             }, 2000);
        //         } else {
