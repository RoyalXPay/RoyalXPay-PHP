...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Offline charity Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/offlinecharity/reports') ?>" class="btn btn-info">Offline charity Payment Reports</a>
        ...
        <div class="card">
            <div class="card-body">
                <form id="offlinecharity-payment-form">
                    <div class="row mb-3">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/offlinecharity/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/offlinecharity/fetch-bill') ?>", {
                    method: 'POST',
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/offlinecharity/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "offlinecharity/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
