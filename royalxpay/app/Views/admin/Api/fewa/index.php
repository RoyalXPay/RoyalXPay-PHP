...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Fewa Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/fewa/reports') ?>" class="btn btn-info">Fewa Payment Reports</a>
...
<script>
        ...
        switch (type) {
            case 'AccountID':
                if ( !/^[0-9]{12}$/.test(value)) {
                    errorEl.text('Account number should be  12 digits.').show();
                    return false;
                }
                break;
            case 'MobileNo':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/fewa/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/fewa/fetch-bill') ?>", {
                    method: 'POST',
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/fewa/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "fewa/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
