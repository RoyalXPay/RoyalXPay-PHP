...
use Mpdf\Mpdf;
class Hafilat extends BaseController
{
    use ResponseTrait;
    ...
    {
        return view('admin/Api/hafilat/index', [
            'pagetitle' => 'Hafilat Payment'
        ]);
    }
...
public function receipt($id)
{
    ...
    try {
        $html = view('admin/Api/hafilate/receipt_pdf', [
            'txn' => $transaction,
            'merchant' => $merchant,
            'customer' => $customer
        ]);
      ...
      if (!$token || !$account || !$walletAmount || !$apiPaidAmount || !$name || !$transactionId) {
        ...
        try {
            ...
            $paymentData = [
                ...
                'reqField1'        => "$account",
                'reqField2'        => "$ProductCode",
                'reqField3'        => "$itemCode",
                'reqField4'        => $customerMobile,
                'reqField5'=> "$name",
                "reqField6"=> "1",
                "reqField7"=> null,
            ];
...
$transactionId = date('ymdHis') . substr(microtime(true) * 100, -2) . rand(0, 9) . '3684';
        ...
        try {
            ...
            $arrayResponse = $balance['responseData']['arrayResponse'][0] ?? [];
            $productCode = $arrayResponse['ProductCode'] ?? '';
            $itemCode = $arrayResponse['ItemInfo'] ?? null; // It is null in your response
   ...
   public function reports()
{
    ...
    return view('admin/Api/hafilat/report', [
        'transactions' => $transactions,
        'pagetitle' => 'Hafilat Payment Reports'
    ]);
}
