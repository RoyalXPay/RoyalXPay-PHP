...
public function fetchNotifications()
{
    ...
    // Step 1: Get all relevant pending wallet entries for this user
    ...
    // Step 2: For each notification, get merchant name from UsersModel
    foreach ($notifications as &$n) {
        $merchant = $userModel->where('user_id', $n['customer_id'])->first();
      
        $n['merchant_name'] = $merchant['name'] ?? 'Unknown';
    }
...
public function fetchNotificationsold()
{
    $session = session();
    $loggedInUserId = $session->get('user_id');
    $userType = $session->get('user_type');
