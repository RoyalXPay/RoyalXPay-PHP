...
<div class="page-content">
                    ...
                    <div class="page-title-right">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <button id="toggleSearchBtn" type="button" class="btn btn-outline-primary waves-effect waves-light">
                               ...
                               <?php if (in_array($user_type, ['superadmin', 'Merchant'])){ ?>
    ...
    <i class="fas fa-file-excel"></i> Bulk Upload
                                ...
                                <table id="datatable" class="table table-bordered dt-responsive w-100">
                                    <thead>
                                        <tr>
                                           ...
                                           <?php if (session('user_type') === 'superadmin'): ?>
                                            <th data-sortable="true" class="text-center">Merchant Name</th>
                                        <?php endif; ?>
                                            ...
                                            <th data-sortable="true" class="text-center">Designation</th>
                                            <th data-sortable="true" class="text-center">Reporting Manager</th>
                                             <th data-sortable="true" class="text-center">Access role</th>
                                            ...
                                            <th data-sortable="true" class="text-center">Maker / Checker</th>
        ...
        <?= esc($item->manager_name) ?><br>
        
    <?php else: ?>
        Not Assigned
    <?php endif; ?>
                                                </td>
                                                 <td class="text-center"><?php echo $item->access_role; ?></td>
                                               ...
                                               <td class="text-center">
                                                ...
                                                <td class="text-center">
    <?php
        if (!empty($item->created_by_type)) {
            echo 'Added by ' . ucfirst($item->created_by_type);
        } else {
            echo 'N/A';
        }
    ?>
    ...
    margin-top: 4px;    width: 36px;"
                                                        class="btn btn-sm toggle-status <?= ($item->status === 'Active') ? 'btn-success' : 'btn-danger'; ?>" 
                                                        data-id="<?= $item->employee_id ?>" 
                                                        data-status="<?= $item->status ?>">
                                                        <?= ($item->status === 'Active') ? '<i class="fas fa-toggle-on"></i>' : '<i class="fas fa-toggle-off"></i>'; ?>
                                                    </button>
                                                    <?php } ?>
                                                         <a href="<?= site_url("employees/view/" . $item->employee_id) ?>" class="btn btn-info m-1" data-bs-toggle="tooltip" title="View Details">
 ...
 <?php if (in_array($user_type, ['maker', 'superadmin', 'merchant'])){ ?>
                                                        ...
                                                        <button type="button" class="btn btn-secondary m-1 view-attendance" data-id="<?= $item->employee_id ?>" data-name="<?= $item->first_name . ' ' . $item->last_name ?>" data-bs-toggle="modal" data-bs-target="#attendanceModal" title="View Attendance">
            ...
            <div class="modal-body">
                <form id="bulkUploadForm" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <input type="file" name="employee_file" class="form-control" accept=".xlsx" required>
                    </div>
                    <div class="mb-3 text-center">
                        <a href="<?= base_url('uploads/Employees-Template.xlsx') ?>" class="btn btn-outline-success" download>
                ...
                <!-- Result will be injected here after upload -->
            </div>
        </div>
    </div>
</div>
...
$(document).on('click', '.toggle-status', function () {
    ...
    $.ajax({
        url: "<?= site_url('employees/update-status') ?>",
        type: "POST",
        data: { employee_id: employeeId, status: newStatus },
        beforeSend: function () {
...
$('#bulkUploadForm').on('submit', function (e) {
    ...
    $.ajax({
        url: "<?= site_url('employees/import-excel') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function () {
            Swal.fire({
                title: 'Uploading...',
                text: 'Please wait while we import employees.',
        ...
        success: function (response) {
            ...
            if (response.uploaded && response.uploaded.length > 0) {
                html += `
                    <h5 class="mb-3 text-success">${response.message}</h5>
                    ...
                    html += `
                        <tr>
                            ...
                            <td class="${emp.status ? 'text-success' : 'text-danger'}">
                                ${emp.status ? 'Inserted' : emp.message}
                            </td>
                        </tr>`;
                });
            ...
            $('#bulkUploadModal .modal-body').html(html);
        },
        error: function () {
            Swal.close();
            Swal.fire('Error', 'Something went wrong. Please try again.', 'error');
...
$(document).on('click', '#cancelUpload', function () {
    window.location.href = "<?= base_url('index.php/employees') ?>";
});
