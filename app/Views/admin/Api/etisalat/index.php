...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Etisalat Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/etisalat/reports') ?>" class="btn btn-info">Etisalat Payment Reports</a>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/etisalat/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/etisalat/fetch-bill') ?>", {
                    method: 'POST',
                ...
                if (data.resField1) {
                    html += `<tr><th>payment Type</th><td>${data.resField1}</td></tr>`;
                }
                if (data.resField2) {
                    html += `<tr><th>Wasel Type</th><td>${data.resField2}</td></tr>`;
                }
                if (data.resField3) {
                    html += `<tr><th>Date</th><td>${data.resField3}</td></tr>`;
                }
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/etisalat/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "etisalat/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
