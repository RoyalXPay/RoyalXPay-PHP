...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Dubai ded Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/dubaided/reports') ?>" class="btn btn-info">dubai ded Payment Reports</a>
...
<script>
    ...
    function validateInput(value, type) {
        ...
        switch (type) {
            case 'AccountID':
                if ( !/^[0-8]{8}$/.test(value)) {
                    errorEl.text('Account number should be  8 digits.').show();
                    return false;
                }
                break;
            case 'MobileNo':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/dubaided/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/dubaided/fetch-bill') ?>", {
                    method: 'POST',
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/dubaided/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "dubaided/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
