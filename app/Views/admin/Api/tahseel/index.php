...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Tahseel Topup Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/tahseel/reports') ?>" class="btn btn-info">Tahseel Topup Payment Reports</a>
        ...
        <div class="card">
            <div class="card-body">
                <form id="tahseel-payment-form">
                    <div class="row mb-3">
                        <label>Select Search Type</label>
                        <div class="col-md-12 d-flex flex-wrap gap-2">
                            <button type="button" class="btn btn-outline-primary search-type-btn" data-type="AccountID">TAHSEEL CARD No</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/tahseel/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/tahseel/fetch-bill') ?>", {
                    method: 'POST',
                ...
                if (data && Array.isArray(data.arrayResponse) && data.arrayResponse.length > 0) {
                    ...
                    const commission = 0;
                    const finalAmount = Math.floor(balanceAmount + commission); // always round down
                    ...
                    // Required Fields First
                        ...
                        if (detail.Balance) {
                            html += `<tr>
                                ...
                                <td>
                                    <input type="number" min="0" step="0.01" id="editableBalance" class="form-control" value="${balanceAmount}" />
                                </td>
                            </tr>`;                        }
                    ...
                    $('#continueBtn').text(`Pay Amount: ${finalAmount}`);
                    $('#continueBtn').data('payload', {
                        ...
                        transaction_id: transactionId,
                        amount: finalAmount,
                        balance: balanceAmount  // for API paidAmount
                ...
                $('#editableBalance').on('input', function () {
                    ...
                    const commission = 0;
                    const finalPayable = (newBalance + commission);

                    $('#finalAmount').val(`${finalPayable} `);
                    $('#continueBtn').text(`Pay Amount: ${finalPayable}`);
    ...
    // Attach mobile to payload
    payload.customer_mobile = mobile;

    try {
    const payRes = await fetch("<?= site_url('api/tahseel/make-payment') ?>", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (payResult.responseCode === '000') {
        ...
        setTimeout(() => {
            window.location.href = "tahseel/reports";
        }, 2000);
    } else {
        Swal.fire('Failed', payResult.error || payResult.billerMessage || 'Transaction failed. Please try again.', 'error');
    }
