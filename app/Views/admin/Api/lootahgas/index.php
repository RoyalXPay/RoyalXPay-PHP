...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">lootahgas Utility Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/lootahgas/reports') ?>" class="btn btn-info">lootahgas Payment Reports</a>
        ...
        <div class="card">
            <div class="card-body">
                <form id="lootahgas-payment-form">
                    <div class="row mb-3">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/lootahgas/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/lootahgas/fetch-bill') ?>", {
                    method: 'POST',
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
        const payRes = await fetch("<?= site_url('api/lootahgas/make-payment') ?>", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const payResult = await payRes.json();
           ...
           if (payResult?.status === 'SUCCESS' || payResult?.data?.status === 'SUCCESS') {
                Swal.fire({
                 ...
                 setTimeout(() => {
                window.location.href = "lootahgas/reports";
            }, 2000);
            } else {
                Swal.fire({
                    icon: 'error',
