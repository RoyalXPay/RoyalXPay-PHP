...
<div class="page-content">
    <div class="container-fluid">
        <h4 class="card-title mb-3">Dubai Payment</h4>
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end gap-2">
                <a href="<?= site_url('api/dubai/reports') ?>" class="btn btn-info">Dubai Payment Reports</a>
...
<script>
    ...
    function validateInput(value, type) {
        ...
        switch (type) {
           case 'AccountID':
    if (!/^002$/.test(value)) {
        errorEl.text('Account number must be exactly 002.').show();
        return false;
    }
    break;
            case 'MobileNo':
    ...
    $(document).ready(function () {
        ...
        $('#payBtn').click(async function () {
            ...
            try {
                const tokenRes = await fetch("<?= site_url('api/dubai/get-token') ?>", { method: 'POST' });
                ...
                const balanceRes = await fetch("<?= site_url('api/dubai/fetch-bill') ?>", {
                    method: 'POST',
...
payload.paidAmount = rawBalance.toFixed(2);
 console.log('Response Data:', payload);
    ...
    try {
    const payRes = await fetch("<?= site_url('api/dubai/make-payment') ?>", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    (payResult.responseCode === '000' && payResult.status === 'SUCCESS')
        ...
        setTimeout(() => {
            window.location.href = "<?= site_url('api/dubai/reports') ?>";
        }, 2000);
    } else if (
