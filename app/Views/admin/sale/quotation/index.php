            ...
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Sale Quotation</a></li>
                        <li class="breadcrumb-item active">Sale Quotation List</li>
                    </ol>

                    <div class="page-title-right">
                        <?php if (!empty($permissions) && in_array('add', $permissions)) { ?>
                            <a href="<?= site_url('sale-quotations/create') ?>" class="btn btn-primary waves-effect waves-light">
                                <i class="mdi mdi-plus"></i> Add Sale Quotation
                            </a>
                        <?php } ?>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-4" style="margin-top: 27px;">
                                                ...
                                                <a href="<?= site_url('sale-quotations'); ?>" class="btn btn-secondary waves-effect waves-light mr-1" data-bs-toggle="tooltip" data-bs-placement="top" title="Clear Searched Filters">
                                    ...
                                    <tbody>
                                        <?php foreach ($results as $items) { ?>
                                            <tr>
                                                ...
                                                foreach ($quotationsProducts as $quotationProductDetails) {
                                                    if ($quotationProductDetails["quotation_id"] == $items->quotation_id) {
                                                        if (!empty($quotationProductDetails["product_master_id"])) {
                                                            foreach ($products as $productDetails) {
                                                                if ($quotationProductDetails["product_master_id"] == $productDetails->product_master_id) {
                                                                    $productNames[] = $productDetails->product_name;
                                                                }
                                                            }
                                                        } elseif (!empty($quotationProductDetails["product_id"])) {
                                                            foreach ($saleProducts as $saleProductDetails) {
                                                                if ($quotationProductDetails["product_id"] == $saleProductDetails->product_id) {
                                                                    $productNames[] = $saleProductDetails->product_name;
                                                                }
                                                            }
                                                        }
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <?php if (!empty($permissions) && in_array('edit', $permissions)) { ?>
                                                            <a href="<?= site_url("sale-quotations/edit/" . $items->quotation_id) ?>" class="btn btn-success m-1 editBtn" data-id="<?= $items->quotation_id ?>" data-bs-toggle="tooltip" title="Edit">
                                                                <i class="fas fa-edit"></i>
                                                            </a>
                                                        <?php } ?>

                                                        <a href="<?= site_url("sale-quotations/preview/" . $items->quotation_id) ?>" class="btn btn-warning m-1" data-bs-toggle="tooltip" title="Preview">
                                                            <i class="fas fa-eye"></i>
                                                        </a>

                                                        <a href="<?= site_url("sale-quotations/generate-quotations/" . $items->quotation_id) ?>" class="btn btn-secondary m-1" data-bs-toggle="tooltip" title="Generate Quotation">
        ...
        $('.dltBtn').click(function(e) {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        type: "GET",
                        url: "<?= site_url('sale-quotations/delete/') ?>" + dataID,
